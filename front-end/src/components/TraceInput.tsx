...
const TraceInput: React.FC<TraceInputProps> = ({ onTrace, isLoading }) => {
  ...
  const [errors, setErrors] = useState<Partial<{ transactionHash?: string }>>({});
  const [transactionHash, setTransactionHash] = useState('');

  const validateForm = (): boolean => {
    const newErrors: Partial<{ transactionHash?: string }> = {};
    ...
    <div className="form-container">
      <div className="form-header">
        <img
          ...
          alt="Hyperliquid Simulations Logo"
      ...
      <form onSubmit={handleSubmit}>
        <div className="form-group">
          ...
          <div style={{ marginTop: '0.5rem', fontSize: '0.9rem', color: 'var(--tn-comment)' }}>
            Enter a transaction hash to debug its step-by-step execution.
          </div>
        </div>
